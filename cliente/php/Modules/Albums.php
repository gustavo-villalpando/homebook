...
final class Albums {
	...
	public function get($public = true) {
		...
		// Albums query
		if ($public===false) $query = Database::prepare(Database::get(), 'SELECT id, title, public, sysstamp, password FROM ? WHERE user_id = ? ' . Settings::get()['sortingAlbums'], array(LYCHEE_TABLE_ALBUMS, $user_id));
